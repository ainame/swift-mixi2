...
For repositories that vendor the upstream as a git submodule, always refresh the submodule against upstream before deciding whether the sync is a no-op. Do not rely only on stale local tags, cached refs, or the currently checked-out gitlink when the task is to check for a newer upstream release.
...
3. Refresh submodule-backed vendors before comparing versions.
   If the authoritative vendor source is a git submodule, initialize it if needed, fetch upstream tags/refs in the submodule, and determine the target tag or commit from upstream before making any no-op decision.
   If the normal automation worktree cannot host nested submodule state, switch to a healthier checkout that owns the repository metadata, refresh the submodule there, and use that refreshed result as the decision source.
   Do not treat an unchanged local gitlink, an old cached tag list, or an uninitialized submodule as sufficient evidence that upstream has not advanced.

4. Diff the vendor spec before changing local code.
...
5. Update the vendored source of truth.
...
6. Regenerate derived code using the repo’s own workflow.
...
7. Reconcile handwritten code with the new surface.
...
8. Verify generator/runtime compatibility explicitly.
...
11. Update `CHANGELOG.md` deliberately when the repo maintains one.
...
## Submodule-first Procedure

Use this procedure whenever the authoritative vendor input lives in a git submodule and the task involves checking for a newer upstream release.

1. Initialize the submodule state with the repo’s supported command, such as `git submodule update --init --recursive`.
2. Refresh the upstream refs from inside the submodule or via the repo’s dedicated update command, such as `git submodule update --init --recursive --remote <path>` or `git -C <path> fetch --tags origin`.
3. Resolve the exact target tag or commit from the refreshed upstream refs.
4. Move the submodule to that target revision and stage the gitlink before code generation.
5. Only after the submodule ref is updated, run the repo’s generation workflow and compatibility fixes.

If the environment blocks network access or upstream auth, say that the upstream freshness check could not be completed. Do not report “already up to date” unless refreshed upstream refs or another equally authoritative source confirm it.
